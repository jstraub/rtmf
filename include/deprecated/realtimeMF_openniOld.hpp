    ...
    uint32_t w, uint32_t h)
{
  ...
  cout<<kRw_<<endl;
  cout<<(kRw_.col(0).cross(kRw_.col(1)) - kRw_.col(2)).transpose()<<endl;
  {
...
//      new pcl::PointCloud<pcl::PointXYZRGB>(*nDisp_));
  const uint32_t w = this->normalsImg_.cols;
  const uint32_t h = this->normalsImg_.rows;
...
//        cout<<j<<" "<<i<<" id="<<idz<<endl;
        zIrgb_.at<cv::Vec3b>(j,i)[0] = mfAxCols(5-idz,0);
        zIrgb_.at<cv::Vec3b>(j,i)[1] = mfAxCols(5-idz,1);
        zIrgb_.at<cv::Vec3b>(j,i)[2] = mfAxCols(5-idz,2);
//        k++;
      }else{
